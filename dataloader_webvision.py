...
class webvision_dataset(Dataset):
    def __init__(self, root_dir, transform, mode, num_class, data_number=0, pred=[], probability=[], log=''):
        self.root = root_dir
        self.transform = transform
        self.mode = mode
        self.number = data_number
        if self.mode == 'test':
            ...
            if self.mode == 'all':
                ...
                if self.mode == 'filter':
                    ...
                    for impath in train_imgs:
                        ...
                        if class_num[label] < self.number:
    ...
    def __getitem__(self, index):
        if self.mode == 'labeled':
            ...
            img2 = self.transform(image)
            return img1, img2
        elif self.mode == 'all' or self.mode == 'filter':
            img_path = self.train_imgs[index]
            target = self.train_labels[img_path]
            image = Image.open(self.root + img_path).convert('RGB')
            img = self.transform(image)
...
class webvision_dataloader():
    def __init__(self, batch_size, num_class, data_number, num_workers, root_dir, log):
    ...
    def run(self, mode, pred=[], prob=[]):
        if mode == 'warmup':
            ...
            eval_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_test,
                                             data_number=self.data_number, mode='all', num_class=self.num_class)
